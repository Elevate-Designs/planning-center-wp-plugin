...
final class PCC_Data {
    ...
    private function normalize_event_instances_response($resp, $public_only) {
        ...
        foreach ($data as $row) {
            ...
            // Public-only filter (heuristic, because Calendar fields can differ across accounts/versions)
            ...
            if ($title === '') {
                // fallback: event instance name if exists
    ...
    /**
     * Heuristic “Public times only / Published” filter:
     * - supports boolean fields or string enums if present.
     * - if no known field exists, assume public (so it won't hide everything unexpectedly).
     */
